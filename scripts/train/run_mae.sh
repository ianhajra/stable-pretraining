#!/usr/bin/env bash
# run_mae.sh — Submit MAE training jobs for all dataset/encoding combos.
#
# Usage:
#   bash scripts/train/run_mae.sh [--script PATH]
#
...
TRAIN_SCRIPT="$SCRIPT_DIR/mae.py"
...
submit_job \
    "mae_gaf_sp500_w$(printf '%03d' $WINDOW_SIZE_GAF)" \
...
submit_job \
    "mae_candle_sp500_w$(printf '%03d' $WINDOW_SIZE_CANDLE)" \
...
header "FF × heatmap"
submit_job \
    "mae_heatmap_ff_w$(printf '%03d' $WINDOW_SIZE_FF)" \
...
echo "  Logs           : logs/mae_*_%j.out / .err"
echo "================================================================"
