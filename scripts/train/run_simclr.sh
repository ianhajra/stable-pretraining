#!/usr/bin/env bash
# run_simclr.sh — Submit SimCLR training jobs for all dataset/encoding combos.
#
# Usage:
#   bash scripts/train/run_simclr.sh [--script PATH]
#
...
TRAIN_SCRIPT="$SCRIPT_DIR/simclr.py"
...
submit_job \
    "simclr_gaf_sp500_w$(printf '%03d' $WINDOW_SIZE_GAF)" \
...
submit_job \
    "simclr_candle_sp500_w$(printf '%03d' $WINDOW_SIZE_CANDLE)" \
...
header "FF × heatmap"
submit_job \
    "simclr_heatmap_ff_w$(printf '%03d' $WINDOW_SIZE_FF)" \
...
echo "  Logs           : logs/simclr_*_%j.out / .err"
echo "================================================================"
