"""DINO training for financial imagery (ViT-Tiny backbone).

Self-distillation with no labels: student trained to match teacher (EMA of student)
across global (224×224) and local (96×96) views.

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_dino.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DINO for financial imagery (ViT-Tiny)"
    )
...
def _make_global_view(transform_names) -> spt_transforms.Compose:
    ...
    if "random_resized_crop" in aug_set:
        steps.append(spt_transforms.RandomResizedCrop((224, 224), scale=(0.4, 1.0)))
    else:
...
def _make_local_view(transform_names) -> spt_transforms.Compose:
    ...
    steps: list = [
        spt_transforms.RGB(),
        spt_transforms.RandomResizedCrop((96, 96), scale=(0.05, 0.4)),
    ]
    _append_photometric_and_domain(steps, aug_set)
    steps.append(spt_transforms.ToImage(mean=IMAGENET_MEAN, std=IMAGENET_STD))
    return spt_transforms.Compose(*steps)
...
# MLP head: 192 → 2048 → 2048 → 256 → L2Norm → prototypes (65536)
# Wrapped so teacher and student share the EMA update path.
projector = nn.Sequential(
    ...
    nn.Linear(256, 65536, bias=False),
)
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"dino_{args.encoding}_{args.dataset}"
...
# the teacher encoder. update_after_backward=False is correct for DINO (unlike IJEPA).
