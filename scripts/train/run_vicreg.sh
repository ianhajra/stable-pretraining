#!/usr/bin/env bash
# run_vicreg.sh — Submit VICReg training jobs for all dataset/encoding combos.
#
# Usage:
#   bash scripts/train/run_vicreg.sh [--script PATH]
#
...
TRAIN_SCRIPT="$SCRIPT_DIR/vicreg.py"
...
submit_job \
    "vicreg_gaf_sp500_w$(printf '%03d' $WINDOW_SIZE_GAF)" \
...
submit_job \
    "vicreg_candle_sp500_w$(printf '%03d' $WINDOW_SIZE_CANDLE)" \
...
header "FF × heatmap"
submit_job \
    "vicreg_heatmap_ff_w$(printf '%03d' $WINDOW_SIZE_FF)" \
...
echo "  Logs           : logs/vicreg_*_%j.out / .err"
echo "================================================================"
