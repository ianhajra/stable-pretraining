"""VICReg training for financial imagery (ViT-Tiny backbone).

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_vicreg.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="VICReg for financial imagery (ViT-Tiny)"
    )
...
# VICReg's variance and covariance terms operate on the full projector output,
# so a wider output is beneficial (same reasoning as Barlow Twins).
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"vicreg_{args.encoding}_{args.dataset}"
