#!/usr/bin/env bash
# run_barlow.sh — Submit Barlow Twins training jobs for all dataset/encoding combos.
#
# Usage:
#   bash scripts/train/run_barlow.sh [--script PATH]
#
...
TRAIN_SCRIPT="$SCRIPT_DIR/barlow.py"
...
submit_job \
    "barlow_gaf_sp500_w$(printf '%03d' $WINDOW_SIZE_GAF)" \
...
submit_job \
    "barlow_candle_sp500_w$(printf '%03d' $WINDOW_SIZE_CANDLE)" \
...
header "FF × heatmap"
submit_job \
    "barlow_heatmap_ff_w$(printf '%03d' $WINDOW_SIZE_FF)" \
...
echo "  Logs           : logs/barlow_*_%j.out / .err"
echo "================================================================"
