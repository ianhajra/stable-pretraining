"""MAE training for financial imagery (ViT-Tiny backbone).

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_mae.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MAE for financial imagery (ViT-Tiny)"
    )
...
# MAE uses a single view — no contrastive pairs needed.
...
# Decoder scaled proportionally from the ViT-Base reference implementation.
module = MAE(
    ...
    decoder_num_heads=4,
    mask_ratio=0.75,
    block_size=1,        # random patch masking
    norm_pix_loss=True,
    loss_type="mse",
    pretrained=False,
)
...
def mae_forward(self, batch, stage):
    ...
    result = {
        "loss": output.loss,
        "embedding": features[:, 1:].mean(dim=1).detach(),   # mean of patch tokens
    }
    if "label" in batch:
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"mae_{args.encoding}_{args.dataset}"
