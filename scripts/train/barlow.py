"""Barlow Twins training for financial imagery (ViT-Tiny backbone).

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_barlow.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Barlow Twins for financial imagery (ViT-Tiny)"
    )
...
# Barlow Twins needs a higher-dimensional projector output than SimCLR so the
# cross-correlation matrix is well-conditioned.
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"barlow_{args.encoding}_{args.dataset}"
