"""SimCLR training for financial imagery (ViT-Tiny backbone).

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_simclr.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SimCLR for financial imagery (ViT-Tiny)"
    )
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"simclr_{args.encoding}_{args.dataset}"
