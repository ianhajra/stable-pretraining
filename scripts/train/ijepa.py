"""I-JEPA training for financial imagery (ViT-Tiny backbone).

Augmentation pipeline is built dynamically from --augmentations; dataset and
encoding are controlled entirely by CLI arguments.  Invoke via run_ijepa.sh
or directly:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="I-JEPA for financial imagery (ViT-Tiny)"
    )
...
# I-JEPA uses a single view — patch masking is handled internally by the model.
...
# Predictor scaled proportionally from the ViT-Base reference:
# predictor_embed_dim = embed_dim / 2  (384 / 768 → 96 / 192)
# predictor_depth halved from ViT-Base's 12.
module = IJEPA(
...
# ── Logger ────────────────────────────────────────────────────────────────────
run_name = (
    f"ijepa_{args.encoding}_{args.dataset}"
...
# TeacherStudentCallback must be first so the EMA update runs before probes read
# the teacher encoder.
teacher_student_cb = spt.callbacks.TeacherStudentCallback(
    update_frequency=1,
    update_after_backward=True,
)

trainer_callbacks = [teacher_student_cb, knn_probe, *probes, rankme, lr_monitor]
