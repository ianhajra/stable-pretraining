"""SigLIP 2 on ImageNet-100 — vision-only self-supervised adaptation.

Trains a ViT-B/16 backbone from scratch using SigLIP 2's multi-positive sigmoid
loss with EMA self-distillation (momentum teacher, similar to BYOL / DINO).

Key differences from SigLIP:
* EMA teacher (TeacherStudentWrapper) provides stable training targets.
* Multi-positive SigLIP2Loss: all augmented crops from the same source image
  are treated as positives, enabling flexible multi-view training.
...
# ---------------------------------------------------------------------------
# Model — ViT-B/16 with EMA teacher
# ---------------------------------------------------------------------------
...
linear_probe = spt.callbacks.OnlineProbe(
    ...
    input="embedding",
    target="label",
    probe=nn.Linear(_embed_dim, 100),
